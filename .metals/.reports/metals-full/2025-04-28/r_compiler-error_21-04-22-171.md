 ...
 */
 ...
 public class SharedMemoryConsumer {
    ...
    public static void dumpBuffer(ByteBuffer buffer, int maxBytes) {
        ...
        buffer.mark(); // save position
        buffer.get(data);
        buffer.reset(); // restore position
    
        System.out.println("[SharedMemory] Raw ByteBuffer content:");
        for (int i = 0; i < data.length; i++) {
            if (i % 16 == 0) System.out.printf("%04X: ", i);
            System.out.printf("%02X ", data[i]);
            if ((i + 1) % 16 == 0) System.out.println();
        }
        if (data.length % 16 != 0) System.out.println();
    }
     ...
     public static <K, V> ConsumerRecords<K, V> readSharedMemoryBySharedMessage(
         ...
         long lastStableOffset = buffer.getLo@@ng(); // unused but parsed
